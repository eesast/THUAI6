    ...
    namespace dragonbox
    {
        ...
        struct cache_accessor<float>
        {
            ...
            static compute_mul_result compute_mul(
                carrier_uint u, const cache_entry_type& cache
            ) noexcept
            {
            ...
            static compute_mul_parity_result compute_mul_parity(
                carrier_uint two_f, const cache_entry_type& cache, int beta
            ) noexcept
            {
                FMT_ASSERT(beta >= 1, "");
                FMT_ASSERT(beta < 64, "");
            ...
            static carrier_uint compute_left_endpoint_for_shorter_interval_case(
                const cache_entry_type& cache, int beta
            ) noexcept
            {
        ...
        struct cache_accessor<double>
        {
            ...
            static uint128_fallback get_cached_power(int k) noexcept
            {
                ...
                recovered_cache =
                    uint128_fallback{(recovered_cache.low() >> alpha) | high_to_middle, ((middle_low.low() >> alpha) | middle_to_low)};
...
namespace detail
{
    ...
    inline auto is_printable(uint16_t x, const singleton* singletons, size_t singletons_size, const unsigned char* singleton_lowers, const unsigned char* normal, size_t normal_size)
        -> bool
    {
        auto upper = x >> 8;
        auto lower_start = 0;

...
#pragma region 躲避
    ...
    if (self != goToRouse || (self == goToRouse && self->studentType != THUAI6::StudentType::Teacher))
    {
        auto trickersVisible = api.GetTrickers();
            ...
            if (goal != emptynode)
            {
                ...
                for (auto& stu : studentVisible)
                {
                        ...
                        && self->playerState != THUAI6::PlayerState::Encouraging && self->playerState != THUAI6::PlayerState::Encouraged)
                    {
                        ...
                        auto m2 = api.Move(20 - double(self->playerID), PI + f * (PI / (3.0 - double(self->playerID - 1.5) * 0.6)) + angleToSelf(stu->x, stu->y));
                        std::this_thread::sleep_for(std::chrono::milliseconds(21));
                    }
                }
                for (auto& tri : trickersVisible)
                {
                    if (disToSelf(tri->x, tri->y) <= 1000 && self->playerState != THUAI6::PlayerState::Rousing
                        && self->playerState != THUAI6::PlayerState::Encouraging && self->playerState != THUAI6::PlayerState::Encouraged)
                    {
                        static int f = 1;
                        api.EndAllAction();
                        if (!canReachAngle(api, self->x, self->y, PI + f * PI / 3 + angleToSelf(tri->x, tri->y), 200, 400)) f = -f;
                        auto m2 = api.Move(20 - double(self->playerID), PI + f * (PI / (3.0 - double(self->playerID - 1.5) * 0.6)) + angleToSelf(tri->x, tri->y));
                        std::this_thread::sleep_for(std::chrono::milliseconds(21));
                    }
                }
                Move(api, self, goal);
...
void AI::play(ITrickerAPI& api)
{
    ...
    int goalclass = 0;
    int goalgate = 0;
    int Accidentnum = Accidentstu.size();
#pragma region 预处理
    api.Print(fmt::format("预处理"));
    static int prepare = 0;
    prepare++;
    if (prepare == 1) Init(api, self);
    if (Nearstu.size() < 5)
    ...
    double Accury0 = abs(disToSelf(Possiblenode0.x, Possiblenode0.y) - neareststu);
    double Accury1 = abs(disToSelf(Possiblenode1.x, Possiblenode1.y) - neareststu);
    double DirectionAccury = abs((a - b) / disToSelf(A.x, A.y));
    api.Print(fmt::format("cos:{}", (a - b) / disToSelf(A.x, A.y)));
    for (auto& stu : studentVisible)
    {
        if (stu->playerState == THUAI6::PlayerState::Addicted)
            ...
            if (prioritystu[stu->playerID] > maxstupri) {
...
#pragma region 优先级
    ...
    if (prepare - Timeofgoalstu > 100
        || disToSelf(Goalstu.x, Goalstu.y)<1000
        || Goalstu.playerState == THUAI6::PlayerState::Addicted)
        Goalstu.playerID = -1;
    api.Print(fmt::format("disofGoalstu:{}", disToSelf(Goalstu.x, Goalstu.y)));
#pragma endregion
#pragma region 防沉迷
    Accidentnum = Accidentstu.size();
    for (int i = 0; i < Accidentnum; i++)
    {
        THUAI6::Student Accstu = Accidentstu.front();
        ...
        if (disToSelf(Accstu.x, Accstu.y) < neareststu + 10)
            neareststu = 25000;
        Accidentstu.push(Accstu);
        Accidentstu.pop();
    }
...
#pragma region 攻击
    ...
    double Angle;
    if (neareststu < 3000 && Viewstu.empty())
    {
        if (self->timeUntilSkillAvailable[0] < 1e-6)
        {
            if (Possiblenode0 != emptynode && Possiblenode1 != emptynode)
            {
                if (api.GetPlaceType(api.GridToCell(Possiblenode0.x), api.GridToCell(Possiblenode0.y)) == THUAI6::PlaceType::Grass)
                {
                    api.UseSkill(0);
                    double Angle = angleToSelf(Possiblenode0.x, Possiblenode0.y);
                    api.Attack(Angle); // 攻击
                }
                else if (api.GetPlaceType(api.GridToCell(Possiblenode1.x), api.GridToCell(Possiblenode1.y)) == THUAI6::PlaceType::Grass)
                {
                    api.UseSkill(0);
                    double Angle = angleToSelf(Possiblenode0.x, Possiblenode0.y);
                    api.Attack(Angle);
                }
                else if (Accury0 > Accury1)
                {
                    api.UseSkill(0);
                    double Angle = angleToSelf(Possiblenode1.x, Possiblenode1.y);
                    api.Attack(Angle);
                }
                else {
                    api.UseSkill(0);
...
#pragma region 追人
    if (!Viewstu.empty())
    {
        api.Print(fmt::format("追逐"));
        if (Goalstu.playerState == THUAI6::PlayerState::Learning
            || Goalstu.playerState == THUAI6::PlayerState::Stunned)
...
#pragma region 丢失视野后寻找
    ...
    {
        api.Print(fmt::format("丢失视野后寻找"));
        api.Print(fmt::format("目标学生：{},更新时间：{} ", Goalstu.playerID, prepare - Timeofgoalstu));
        api.Print(fmt::format("学生坐标：{},{} ", Goalstu.x / 1000, Goalstu.y / 1000));
        for (int i = 0; i < 8; i++)
        {
            node a = node(api.GridToCell(Goalstu.x), api.GridToCell(Goalstu.y)) + dir[i];
            int dis = disToSelf(api.CellToGrid(a.x), api.CellToGrid(a.y));
            double angle = angleToSelf(api.CellToGrid(a.x), api.CellToGrid(a.y));
            if (!api.HaveView(api.CellToGrid(a.x), api.CellToGrid(a.y)) && canMove[a.x][a.y])
            {
                if (api.GetPlaceType(a.x, a.y) == THUAI6::PlaceType::Window
                    || api.GetPlaceType(a.x, a.y) == THUAI6::PlaceType::Grass)
                    goal = a;
            }
        }
...
#pragma region 通过bgm寻找可能学生
    else if (neareststu < 7000 || (DirectionAccury>0.8 && DirectionAccury<1))
    {
        api.Print(fmt::format("bgm"));
        if (Accury0 < 1000 || Accury1 < 1000 && Possiblenode0 != emptynode && Possiblenode1 != emptynode)
        {
            if (Accury0 < Accury1) goal = node(api.GridToCell(Possiblenode0.x), api.GridToCell(Possiblenode0.y));
...
#pragma endregion
    api.Print(fmt::format("goal：{},{}", goal.x, goal.y));
    Move(api, self, goal);
}

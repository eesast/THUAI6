...
double F_parameters[5][7] = { {0, 0, 10000, 0, 0, 0, 0},
    {0, 1, 10000, 1, 1, 1, 1},
    {0, 20000, 0, 20000, 20000, 20000, 20000},
    {0, 200, 0, 200, 200, 200, 200},
    {0, 0.4, 0, 0.4, 0.4, 0.4, 0.4} };
...
XYSquare BGM_Utilize::GradientAim(bool aimrow) const
{
    ...
    if (abs(grads[0]) < 0.02 && abs(grads[1]) < 0.02)return IdleAim;
    double asylum = SeekDir(dirs[0], dirs[1], grads[0], grads[1]);//Direction of ascending BGM
    XYGrid aiming;
    if(aimrow)aiming = XYGrid((int)(selfInfoTricker->x + 20000 / (bgm[1][2] + 1) * cos(asylum)),
        (int)(selfInfoTricker->y + 20000 / (bgm[1][2] + 1) * sin(asylum)));
    else aiming = XYGrid((int)(selfInfoTricker->x + 20000 / (bgm[0][2] + 1) * cos(asylum)),
        (int)(selfInfoTricker->y + 20000 / (bgm[0][2] + 1) * sin(asylum)));   
    while ((aiming.x >= 1000 && aiming.x < 49000 && aiming.y >= 1000 && aiming.y < 49000) &&
        (oriMap[SquareToCell(GridToSquare(aiming)).x][SquareToCell(GridToSquare(aiming)).y] != THUAI6::PlaceType::Grass
...
void Move(IAPI& api, XYSquare toMove)
{
    ...
    int y = SquareToGrid(toMove).y; api.Print("AboutToMove\n");
    api.Move(60, atan2(y - selfInfo->y, x - selfInfo->x)); api.Print("Moved\n");
}
...
XYSquare FindMoveNext(XYSquare toMove)
{
    ...
    while (x != selfSquare.x || y != selfSquare.y)
    {
        ...
        x = fromMap[lx][ly].x;
        y = fromMap[lx][ly].y; 
        if (x == lx && y == ly && (x != selfSquare.x || y != selfSquare.y)) {  break; }
    }
...
void Update(const ITrickerAPI& api)
{
    ...
    if (stu.size() > 0)
    {
        for (int i = 0; i < stu.size(); ++i)
        {
            ...
            if (Trickers_Students[stu[i]->playerID].addiction > 0)
...
{
    int attackrange = int((selfInfo->bulletType == THUAI6::BulletType::CommonAttackOfTricker) ? 
        Constants::CommonAttackOfTricker::BulletAttackRange : Constants::FlyingKnife::BulletAttackRange);
    ...
    if ((Trickers_Students[maxstudent].playerstate != THUAI6::PlayerState::Addicted && Trickers_Students[maxstudent].playerstate != THUAI6::PlayerState::Roused)
        &&  (aimed == THUAI6::PlaceType::Grass || aimed == THUAI6::PlaceType::Land))
    {
...
void UpdateF(ITrickerAPI& api, int number)
{
    ...
    if (Trickers_Students[number].career == THUAI6::StudentType::Teacher)
    {
        if (api.GetGameInfo()->studentGraduated + api.GetGameInfo()->studentQuited == 3)  // Only one left in play
        {
            ...
            Trickers_Students[number].UpdateFValue(0); api.Print("931\n");
    }
    else
    {
        int i = 0, j = 0;
        double smDist = dist(StudentToMachine);
        double stDist = dist(StudentToTricker); api.Print("937\n");
...
//    {
//        XYCell aim(SquareToCell(GridToSquare(XYGrid((int)(round(Trickers_Students[guard].GetLatestCooridinates().x + Trickers_Students[guard].radius * 3 * cos(theta))),
//            int(round(Trickers_Students[guard].GetLatestCooridinates().y + Trickers_Students[guard].radius * 6 * sin(theta)))))));
//        if (aim.x <= 0)return XYSquare(-1, -1);
//        THUAI6::PlaceType aimed = oriMap[aim.x][aim.y]; api.Print("1055\n");
...
//            aim = SquareToCell(GridToSquare(XYGrid(int(round(Trickers_Students[guard].GetLatestCooridinates().x + Trickers_Students[guard].radius * 3 * cos(theta))),
//                int(round(Trickers_Students[guard].GetLatestCooridinates().y + Trickers_Students[guard].radius * 6 * sin(theta))))));
...
//        XYGrid arr1(selfInfo->x - Trickers_Students[guard].GetLatestCooridinates().x, selfInfo->y - Trickers_Students[guard].GetLatestCooridinates().y);
//        double theta1 = atan2(arr1.y, arr1.x);
//        XYCell aim(SquareToCell(GridToSquare(XYGrid(int(round(Trickers_Students[guard].GetLatestCooridinates().x + Trickers_Students[guard].radius * 3 * cos(0.5 * theta + 0.5 * theta1))),
//            int(round(Trickers_Students[guard].GetLatestCooridinates().y + Trickers_Students[guard].radius * 3 * sin(0.5 * theta + 0.5 * theta1)))))));
...
//            aim = SquareToCell(GridToSquare(XYGrid(int(round(Trickers_Students[guard].GetLatestCooridinates().x + Trickers_Students[guard].radius * 3 * cos(0.5 * theta + 0.5 * theta1))),
//                int(round(Trickers_Students[guard].GetLatestCooridinates().y + Trickers_Students[guard].radius * 3 * sin(0.5 * theta + 0.5 * theta1))))));
//            aimed = oriMap[aim.x][aim.y];
//        }
...
XYSquare SeekDisappearSquare(ITrickerAPI& api, short target)
{
    ...
    {
        XYGrid movement[3]; api.Print("1086\n");
    ...
    if (suppose.x < 0)return XYSquare(-1, -1);//ERROR
...
XYSquare SeekInvisibleStudent(ITrickerAPI& api)
{
    XYSquare ret(-1, -1);
    double grads[2] = { 0 };
    BGM.GetField(true, grads); api.Print("SeekInvisibleStudentBegin\n");
    if (abs(grads[0]) < 0.02 && abs(grads[1]) < 0.02)//No significant change
    {
        BGM.GetField(false, grads); api.Print("Field false Got\n");
        if (abs(grads[0]) < 0.02 && abs(grads[1]) < 0.02)//Still no change
        {
            XYGrid aiming(2 * selfInfo->x - 25000, 2 * selfInfo->y - 25000); api.Print("Seekinvisiblestudent->nochange");
                    ...
                    && oriMap[SquareToCell(GridToSquare(aiming)).x][SquareToCell(GridToSquare(aiming)).y] != THUAI6::PlaceType::Land))
            {
                ...
                api.Print("seekinvisible -> Aiming further\n");
            }
            if (!(oriMap[SquareToCell(GridToSquare(aiming)).x][SquareToCell(GridToSquare(aiming)).y] != THUAI6::PlaceType::Grass &&
                oriMap[SquareToCell(GridToSquare(aiming)).x][SquareToCell(GridToSquare(aiming)).y] != THUAI6::PlaceType::Land))
                return GridToSquare(CellToGrid(SquareToCell(GridToSquare(aiming))));  // Repeated procedures turn target to center of the cell
        ...
        {
            ret = BGM.GradientAim(false); api.Print("SeekInvisibleStudent->if->else\n");
        }
    }
    else
    {
        ret = BGM.GradientAim(true); api.Print("Seekinvisiblestudent->GradientAim");
    }
    return ret;
}
...
{
    if ((IdleAim.x > 0 && IdleAim.y > 0 && oriMap[SquareToCell(IdleAim).x][SquareToCell(IdleAim).y] != THUAI6::PlaceType::Land &&
        oriMap[SquareToCell(IdleAim).x][SquareToCell(IdleAim).y] != THUAI6::PlaceType::Grass)||IdleAim.x<0||IdleAim.y<0)
    {
    ...
    else if (dist(XYGrid(SquareToGrid(IdleAim).x - selfInfo->x, SquareToGrid(IdleAim).y - selfInfo->y)) < 1000)
    {
        ...
        {
            IdleAim = FindClassroom(); api.Print("FindClassroomCalled.\n");
    ...
    {
        IdleAim = FindClassroom(); api.Print("TryToInit\n");
        if (dist(XYGrid(SquareToGrid(IdleAim).x - selfInfo->x, SquareToGrid(IdleAim).y - selfInfo->y)) < 1000)
        {
            XYSquare IdleAim1 = SeekInvisibleStudent(api);
            if (IdleAim1.x > 0)
            ...
            else
            {
                ...
                if(IdleAim.x > 0)
                    ...
                    IdleAim = GridToSquare(CellToGrid(XYCell(25, 13)));
            }
            TrickerIdlePhase = 1;
    ...
    else
    {
        ...
        {
            bool moved = false;
            BGM.UpdateBGM(selfInfoTricker->trickDesire, selfInfoTricker->classVolume); api.Print("Idlephase=-1");
            XYSquare IdleAim1 = SeekInvisibleStudent(api); api.Print("InvisibleStudentSought\n");
            if (IdleAim1.x > 0)
            {
        ...
        else if (TrickerIdlePhase == 8)//Approaching end of a period
        {
            ...
            Move(api, FindMoveNext(IdleAim)); api.Print("Idlephase=8\n");
            TrickerIdlePhase = 9;
        }
        else if (TrickerIdlePhase == 9)//At the end of a period
        {
            BGM.UpdateBGM(selfInfoTricker->trickDesire, selfInfoTricker->classVolume); api.Print("IdlePhase=9\n");
            if (BGM.Decreasing())
            {
                bool moveable = true;
                if (abs(selfInfo->facingDirection) < 0.02 || abs(selfInfo->facingDirection) > 3.123)
                    TrickerIdlePhase = 0;
                else if (oriMap[SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).x]
                    [SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).y + 1] == THUAI6::PlaceType::Land)
                    api.MoveRight(50);
                else if (oriMap[SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).x + 1]
                    [SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).y] == THUAI6::PlaceType::Land)
                    api.MoveDown(50);
                else if (oriMap[SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).x]
                    [SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).y - 1] == THUAI6::PlaceType::Land)
                    api.MoveLeft(50);
                    ...
                    [SquareToCell(GridToSquare(XYGrid(selfInfo->x, selfInfo->y))).y] == THUAI6::PlaceType::Land)
                    api.MoveUp(50);
                else moveable = false; api.Print("EndTestRoute\n");
                if (moveable)
                    TrickerIdlePhase = -1;
                else
                    TrickerIdlePhase = 0;
            ...
            {
                Move(api, FindMoveNext(IdleAim));
                TrickerIdlePhase = 0; api.Print("BGM Increasing\n");
            }
        }
        else //Within a period
        {
            ...
            TrickerIdlePhase = (TrickerIdlePhase + 1) % 10; api.Print("Intra-period mod\n");
        }
...
void AI::play(ITrickerAPI& api)
{
    ...
    if (Commandable(api))
    {
        ...
        //    short approachingStudent = FindNextMaxStudent(api, fixation); api.Print("1259\n");
        ...
        //        {
        //            XYSquare toGuardSquare = FindGuardSquare(api, fixation, approachingStudent); api.Print("1266\n");
        ...
        //            {
        //                Move(api, FindMoveNext(toGuardSquare)); api.Print("Guard pos found\n");
        //            }
        //            else api.Print("Guard pos not found\n");
        //        }
        //    }
        ...
        //            TrickerStatus = 0; api.Print("1275\n");
        //            fixation = -1;
        //            Idle(api);
        //        }
        //        else if (fixation1 >= 0)
        ...
        //    {
        //        TrickerStatus = 1; api.Print("1281\n");
        //    }
        //}
        //else
        {api.Print("1285\n");
            ...
            {api.Print("1295\n");
                if (maxstudent != fixation)  // Target changed
                {
                    if (fixation >= 0)
                        Trickers_Students[fixation].isFixed = false;
                    fixation = maxstudent; api.Print("1300\n");
                ...
                {api.Print("1306\n");
                    if (TryToAttack(api, maxstudent))  // Try to attack maxstudent
                        ...
                        // }
                    XYSquare toStudentSquare = FindStudentSquare(maxstudent); api.Print("1315\n");
                    ...
                    {
                        Move(api, FindMoveNext(toStudentSquare)); api.Print("1318\n");
                    }
                    TrickerStatus = 1;
                }
            }
            ...
            {api.Print("1325\n");
                if (fixation >= 0)  // Existent in last run, target lost
                {
                    ...
                    //}
                    TrickerIdlePhase = -1;
                    TrickerStatus = 0; api.Print("Student lost\n");
                    fixation = -1;
                }
            }
        }api.Print("1345\n");
